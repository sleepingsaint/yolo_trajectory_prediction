...
def main():
    ...
    parser.add_argument('--name', type=str, default="my_data_train_meanstd")
    ...
    for i in np.unique(train_dataset[:]['dataset']):
        ...
        means.append(torch.cat((train_dataset[:]['src'][ind, 1:, 3:6], train_dataset[:]['trg'][ind, :, 3:6]), 1).mean((0, 1)))
        stds.append(
            torch.cat((train_dataset[:]['src'][ind, 1:, 3:6], train_dataset[:]['trg'][ind, :, 3:6]), 1).std((0, 1)))
    ...
    while epoch<args.max_epoch:
        ...
        for id_b,batch in enumerate(tr_dl):
            ...
            # print(pred)
            loss = F.pairwise_distance(pred[:, :,0:3].contiguous().view(-1, 2),
                                       ((batch['trg'][:, :, 3:6].to(device)-mean.to(device))/std.to(device)).contiguous().view(-1, 2).to(device)).mean() + torch.mean(torch.abs(pred[:,:,3]))
            loss.backward()
            optim.step()
