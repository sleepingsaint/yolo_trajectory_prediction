...
def get_strided_data_clust(dt, gt_size, horizon, step):
    ...
    ped = raw_data.ped.unique()
    # print(ped)
    #print(raw_data[raw_data.ped == 2].shape[0])
    #print(dt[dt.ped == 2].iloc[:,[0]].values.squeeze())
    frame=[]
    ped_ids=[]
    for p in ped:
        for i in range(1+(raw_data[raw_data.ped == p].shape[0] - gt_size - horizon) // step):
            #print(p)
            #print(raw_data[raw_data.ped == p].iloc[i * step:i * step + gt_size + horizon, 2:4].values)
            #print("Hi")
            frame.append(dt[dt.ped == p].iloc[i * step:i * step + gt_size + horizon, [0]].values.squeeze())
            # print("%i,%i,%i" % (i * 4, i * 4 + gt_size, i * 4 + gt_size + horizon))
            inp_te.append(raw_data[raw_data.ped == p].iloc[i * step:i * step + gt_size + horizon, 2:5].values)
            ped_ids.append(p)
        #print(inp_te)
        #print("hii")
    #print(len(frame))
...
if __name__ == '__main__':
    create_dataset('datasets','zara1',0,8,12,delim="\t",train=True,eval=False,verbose=False)
