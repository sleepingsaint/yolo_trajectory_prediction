...
def main():
    ...
    model.load_state_dict(torch.load(f'models/Individual/my_data_train/00002.pth'))
    ...
    print("mad %i fad %i"%(mad,fad))
    for i in range(pr.shape[0]):
        pathin = 'C:/Users/venny/Desktop/new data/5_1 frames/5_1_'
        pathout = 'C:/Users/venny/Desktop/new data/5_1 frames_out/'
        img = cv2.imread(pathin+str(frames[i][8])+'.jpg')
        cg = (0,255,0)
        cp = (0,0,255)
        for j in range(12):
            gp = (int(gt[i,j,0]*1080),int(gt[i,j,0])*1920)
            pp = (int(pr[i,j,0]*1080),int(pr[i,j,0])*1920)
            img = cv2.circle(img,gp,0,cg,-1)
            img = cv2.circle(img,pp,0,cp,-1)
            print(gp)
            print(pp)
